package ru.stepanov.pharmacymanager.gui.entitytable;
...
import ru.stepanov.pharmacymanager.gui.entitytable.filter.ScrollPaneTableFilter;
import ru.stepanov.pharmacymanager.gui.entitytable.model.EntityTableModel;
...
public class EntityTable<T> extends JScrollPane {
   ...
   * @param filter фильтр
   */
  public void filter(ScrollPaneTableFilter<T> filter) {
    if (!filter.isEmpty()) {
      model.filter(filter);
    } else {
      model.clearFilter();
